...
void all_data_update(void)
{
    ...
    // mcu_dp_string_update(DPID_A,当前日志上传指针,当前日志上传数据长度); //STRING型数据上报;
...
static unsigned char dp_download_meal_plan_handle(const unsigned char value[], unsigned short length)
{
    // 示例:当前DP类型为RAW
    unsigned char ret;
    // 如果length == 1，则代表删除了所有时间点，则直接清空所有时间点
    LOG_DEBUG("dp_download_meal_plan_handle: length: %d", length);
    if (length == 1)
    {
    ...
    // 其余length都是5的倍数
    // 如7f 10 12 01 01 7f 10 14 01 01代表了两个时间点，周一到周日都生效，16：18，喂食1份，开启。周一到周日都生效，16：20，喂食1份，开启。
    // 所以需要遍历value，每次取5个字节，然后处理
    else
    {
        /* 处理喂食计划：遍历每一位，为所有设置为1的星期添加/删除时间点 */
        // weekday_bits的bit映射：bit 6=周一, bit 5=周二, bit 4=周三, bit 3=周四, bit 2=周五, bit 1=周六, bit 0=周日
        // 转换为RTC模块的weekday：1=周一, 2=周二, 3=周三, 4=周四, 5=周五, 6=周六, 7=周日
        /* 先清空所有星期的时间点，避免残留 */
        for (uint8_t weekday = 1; weekday <= 7; weekday++)
        {
            RTC_Motor_ClearDaySchedule(weekday);
        ...
        for (uint8_t i = 0; i < length; i += 5)
        {
            // RAW type data processing
            //  上述举例，对于APP端时间下发的结果为7f 12 32 0c 01，表示周1-7都生效，18：50，喂食12份，开启。
            // 7F0A0F0c01 周一到周日都生效，10：15，喂食12份，开启。
            //  value[0]的前7个比特位代表周一到周日：
            //  bit 6 = 周一, bit 5 = 周二, bit 4 = 周三, bit 3 = 周四, bit 2 = 周五, bit 1 = 周六, bit 0 = 周日
            //  例如：0x40 (01000000) = 周一, 0x60 (01100000) = 周一和周二
            uint8_t weekday_bits = value[i];
            uint8_t hour = value[i + 1];
            uint8_t minute = value[i + 2];
            uint8_t feed_amount = value[i + 3];
            uint8_t switch_state = value[i + 4];
            // LOG_DEBUG("dp_download_meal_plan_handle: weekday_bits: %d, hour: %d, minute: %d, feed_amount: %d, switch_state: %d", weekday_bits, hour, minute, feed_amount, switch_state);

            /* 再根据位图重新添加需要的时间点 */
            for (uint8_t bit_pos = 0; bit_pos < 7; bit_pos++)
            {
                if ((weekday_bits >> bit_pos) & 0x01)
                {
                    /* bit 6 -> weekday 1, bit 5 -> weekday 2, …, bit 0 -> weekday 7 */
                    uint8_t weekday = 7 - bit_pos;

                    if (switch_state != 0U)
                    {
                        uint8_t result = RTC_Motor_AddTimePoint(weekday, hour, minute);
                        (void)result;
                    }
                    else
                    {
                        RTC_Motor_RemoveTimePoint(weekday, hour, minute);
                        // LOG_DEBUG("RTC_Motor_RemoveTimePoint: weekday: %d, hour: %d, minute: %d", weekday, hour, minute);
                    }
                }
            }
        }
        /* 触发Flash写入请求（实际写入在任务中执行） */
        Flash_Schedule_RequestWrite();
    }
...
void mcu_write_rtctime(unsigned char time[])
{
   ...
   */
    // 0x01 0x19 0x0B 0x1A 0x09 0x36 0x21 0x03
    // 成功 2025年 11月 25日 9时 54分 33秒 星期三
    if (time[0] == 1)
    {
        // 正确接收到wifi模块返回的本地时钟数据
        RTC_User_SetDate(time[1] + 2000, time[2], time[3], time[7]); // 设置日期和星期
        RTC_User_SetTime(time[4], time[5], time[6]);                 // 设置时间
        // update_next_alarm();
    }
    else
    {
